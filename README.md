...
```bash
go install github.com/y-miyazaki/absc/cmd/absc@v1.0.8
```

### Using Release tar.gz

You can download a prebuilt release tarball from the Releases page and install it quickly. The examples below use the `v1.0.8` release.
...
```bash
VERSION=v1.0.8 && curl -L https://github.com/y-miyazaki/absc/releases/download/${VERSION}/absc-linux-amd64.tar.gz | tar -xzf - && sudo mv absc /usr/local/bin/ && sudo chmod +x /usr/local/bin/absc
```

macOS (ARM64) example:

```bash
VERSION=v1.0.8 && curl -L https://github.com/y-miyazaki/absc/releases/download/${VERSION}/absc-darwin-arm64.tar.gz | tar -xzf - && sudo mv absc /usr/local/bin/ && sudo chmod +x /usr/local/bin/absc
```
...
- `go install` is convenient for development. Release tarballs are preferable when you want a pinned binary such as `v1.0.8`.
...
jobs:
  collect:
    ...
    steps:
      ...
      - name: Install absc
        run: go install github.com/y-miyazaki/absc/cmd/absc@v1.0.8
