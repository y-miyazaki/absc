...
func TestPageSizeForLimit(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "use service max when limit is zero", limit: 0, serviceMax: 200, want: 200},
...
func TestRemainingPageSize(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "fallback to service max when limit unset", limit: 0, collected: 99, serviceMax: 200, want: 200},
	}
